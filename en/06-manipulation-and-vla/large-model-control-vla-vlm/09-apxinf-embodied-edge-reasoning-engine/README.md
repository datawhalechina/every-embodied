...
Source SHA-256: a6048178517f0bd474317e3b774882ff2b9fbb188094b768d15d0f2c3d24b089
...
## 1. Why VLA requires a dedicated end-side Runtime

The training load and robot deployment load for VLA vary significantly. During the training phase, large memory GPUs and larger batches are commonly used; during deployment, Jetson Orin, Jetson Thor, or a single RTX 4090 is typical, with a batch size of 1. The following closed loop must be repeatedly executed:
...
The release focus for the official repository is π0.5, and it offers paths for BF16, FP8, and INT8. Currently supported hardware includes Jetson AGX Thor, Jetson AGX Orin, and RTX 4090; FP8 requires calibration data, while INT8 is primarily aimed at Orin and Ada architectures. The specific support scope should be based on the README of the target version.

### Table 1: Boundaries of Related Projects

| Item | Primary Location | Equivalent to APXInf? |
| --- | --- | --- |
| RLinf | embodied RL training, rollout, simulation, and evaluation infrastructure | No, it belongs to the training/evaluation layer |
| OpenPI | π0/π0.5 model, preprocessing, and policy service reference implementations | No, it belongs to the model and policy stack |
| APXInf | real robot-side inference Runtime for small batches and low latency embodied models | Main content of this chapter |
| EVA-Client | deployment, collection, and evaluation client for connection policy services and real robots | No, it belongs to the real robot execution layer |
| Jetson-PI | research approach for hiding VLA latency through asynchronous inference and predictive correction | No, it belongs to control/scheduling methods |
...
The papers and code of EVA-Client can be found in the [ project repository ](https://github.com/Noietch/EVA-CLIENT) and the [ papers ](https://arxiv.org/abs/2607.02646); the papers and Edge Runtime of Jetson-PI can be found in the [ paper ](https://arxiv.org/abs/2607.12659); Embodied.cpp can be found in the [ project repository ](https://github.com/SEU-PAISys/Embodied.cpp) and the [ papers ](https://arxiv.org/abs/2607.02501). These projects have different purposes, and we cannot consider them as the same framework just because they both mention "inference".

## 3. Things Worth Looking At About APXInf

### 3.1 Focus on closed loop at the edge, rather than optimizing the bare model only

The public benchmark for APXInf divides the measurements into L1, L2, and L3: L1 refers to the transformation from resized RGB to model output, L2 includes the preprocessing and post-processing of the policy, and L3 also encompasses server-side round trips. This segmentation helps identify whether the bottleneck lies in CUDA computation, Python/Rust calls, policy encapsulation, or communication links.
...
The 26.32 ms for Thor FP8 corresponds to approximately 38 Hz, indicating that its optimization goal is indeed real-time robot control, rather than solely focusing on offline throughput. [ Official performance table ](https://github.com/RLinf/APXinf-robo#performance)

### 3.2 Clear Division of Labor between Rust Runtime and CUDA Kernel

The Python API is responsible for model invocation and upper-level integration, Rust handles the runtime organization that is closer to the system, and CUDA Kernel handles GPU computing. Rust is not the embodied AI algorithm itself; its value lies in reducing runtime resource management risks, converging interfaces, and minimizing scheduling overhead. The real performance benefits still come from graph capture, fusion operators, precision paths, memory reuse, and model structure specialization.
...
The official build process requires Linux, NVIDIA drivers, CUDA Toolkit, Rust, and CMake. The compilation process reads the compute capability of the currently visible GPU and builds a CUDA Kernel for the target architecture; therefore, it is best to build directly on the deployment machine. When building across machines, `APXINF_CUDA_ARCH` must be explicitly set, for example, Orin uses `sm_87`.
...
Random weight benchmark is suitable for checking CUDA Kernel, input shape, image capture, and timing links. It does not verify model performance, and cannot replace LIBERO or real robot evaluations with actual checkpoints.

### Checkpoint 3: Use a real checkpoint

The actual evaluation requires a compatible π0.5 checkpoint, as well as the corresponding tokenizer, normalization statistics, and model configuration. The LIBERO process described in the official README requires preparing `norm_stats.json` additionally, and running 10 tasks with `apxinf-robo eval-libero` for evaluation. It is recommended to run a single-task single-round smoke test first, and then conduct the full evaluation:
...
If the smoke test passes, replace `--tasks 0 --trials-per-task 1` with the complete task and number of rounds. The complete success rate should be compared with the reference results under the same checkpoint, normalization, and seed conditions; do not compare only one demo video.
...
- CUDA Graph usually requires stable input shapes and execution paths. Dynamic number of viewpoints, dynamic action horizon, or temporary changes to flow steps may lead to re-acquisition or loss of optimization benefits.
- End-to-end latency should be broken down into model computation, preprocessing, post-processing, transmission, and robot execution. Reporting only L1 latency tends to overestimate the actual closed loop frequency.

## 6. How to Understand Its Innovation

The main contribution of APXInf is in systems engineering, not a new VLA training algorithm. It does not alter the task definition of π0.5, nor introduces a new visual encoder or action learning objective; instead, it applies the already trained policy to the robot and makes the entire execution path more compact under fixed shape, small batch, and real-time closed loop conditions.
...
- Generalizability: still limited by the scope of model and hardware support;
...
The actual system does not necessarily have to choose one of these options:
